...
import {compare, arrayify, process} from './lib'

import fixture from './test/fixture.json'

test('process strips strings as necessary', function(t) {
  t.equal(process('The Thing'), 'Thing')
  t.equal(process('A Thing 1'), 'Thing 1')
  t.equal(process('a thing and an apple'), 'thing and an apple')
  t.equal(process(1), '1')
  t.equal(process('a good   thing'), 'good thing')
  t.equal(process('good   thing  too'), 'good thing too')
  t.equal(process('(1997)'), '1997')

  t.end()
})

test('arrayify makes arrays from not arrays', function(t) {
  t.deepEqual(arrayify([]), [])
  t.deepEqual(arrayify(1), [1])
  t.deepEqual(arrayify('something'), ['something'])
  t.deepEqual(arrayify(['a', 'b']), ['a', 'b'])
  t.deepEqual(arrayify({beep: 'boop'}), [{beep: 'boop'}])
